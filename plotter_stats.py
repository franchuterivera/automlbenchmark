...
def parse_data(csv_location: str) -> pd.DataFrame:
    ...
    """
    data = []
    for data_file in glob.glob(os.path.join(csv_location, '*stat*.csv')):
        data.append(
            pd.read_csv(
                data_file,
                index_col=0,
    ...
    data = pd.concat(data).reindex().infer_objects()

    # Make sure our desired columns are numeric
    #data['test'] = pd.to_numeric(data['test'])

    required_columns = ['task', 'fold']

    # A final sort
    data = data.sort_values(by=['framework']+required_columns).reset_index(drop=True)

...
data = []
#for data_file in glob.glob('misc/*07_11_2020_overfit_status*csv'):
...
experiment_results = {}
for tool_task, overfit_value in data.groupby(['tool', 'task']).mean()['overfit'].to_dict().items():
    tool, task = tool_task
    if tool not in experiment_results:
        experiment_results[tool] = {}
    if task not in experiment_results[tool]:
        experiment_results[tool][task] = overfit_value

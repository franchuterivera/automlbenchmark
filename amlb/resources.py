    ...
    def _validate_task(self, task, lenient=False):
        ...
        if task[conf] is None:
            task[conf] = ("openml.org/t/{}".format(task.openml_task_id) if task['openml_task_id'] is not None
                          ...
                          else "openml.org/k/{}".format(task.kaggle_path) if task['kaggle_path'] is not None
            ...
            if not lenient and task[conf] is None:
                raise ValueError("task definition must contain one property among ['openml_task_id', 'dataset', 'kaggle_path']")

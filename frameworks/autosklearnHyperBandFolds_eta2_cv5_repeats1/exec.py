...
def get_smac_object_callback(cv_folds, eta, repeats):
    def get_smac_object(
        scenario_dict,
        level,
        seed,
...
def run(dataset, config):
    ...
    folds_to_use = 'highest_fold_available'
    if 'folds_to_use' in training_params:
        folds_to_use = training_params.pop('folds_to_use')
    eta = 2
    if 'eta' in training_params:
        folds_to_use = training_params.pop('eta')
    repeats = 1
    ...
    auto_sklearn = estimator(time_left_for_this_task=config.max_runtime_seconds,
                             ...
                             max_stacking_levels=1,
                             resampling_strategy_arguments={'folds': k_folds, 'folds_to_use': 'highest_fold_available', 'repeats': repeats},
                             get_smac_object_callback=get_smac_object_callback(k_folds, eta, repeats),
                             seed=config.seed,
                             **constr_extra_params,
                             **training_params)

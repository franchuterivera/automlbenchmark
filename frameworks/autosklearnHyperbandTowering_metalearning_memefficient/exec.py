...
def get_smac_object_callback(repeats, level):
    def get_smac_object(
        scenario_dict,
        run_id,
        seed,
        ...
        from smac.intensification.towering import TowerHyperband
        ...
        return SMAC4AC(
            ...
            run_id=run_id,
            intensifier=TowerHyperband,
            intensifier_kwargs={
                'min_chall': 1,
                'initial_budget': 1,
                'max_budget': repeats*level,
                # This is changed in smac -- but I hope it is not an issue
                # it is changed to shuffle once
                'instance_order': None,
            },
            n_jobs=n_jobs,
...
def run(dataset, config):
    ...
    if 'avg_previous_repeats' in training_params:
        ...
        if isinstance(avg_previous_repeats, str):
            if 'True' in avg_previous_repeats:
                ...
                avg_previous_repeats = True
    print(f"Using avg_previous_repeats={avg_previous_repeats} ({type(avg_previous_repeats)})")
    log.warning(f"Using avg_previous_repeats={avg_previous_repeats}")
    stack_at_most = None
    ...
    auto_sklearn = estimator(time_left_for_this_task=config.max_runtime_seconds,
                             ...
                             resampling_strategy_arguments={'folds': k_folds,
                                                            ...
                                                            'repeats': repeats},
                             get_smac_object_callback=get_smac_object_callback(1 if train_all_repeat_together else repeats, max_stacking_level),
                             seed=config.seed,
                             **constr_extra_params,
                             **training_params)
    ...
    try:
        ...
        print(f"Faile predicting, save things to debug")
        print(e)

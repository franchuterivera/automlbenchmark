...
def run(dataset, config):
    ...
    input_dir = training_params.pop('input_dir')
    ...
    if len(input_dir) != 1 or not os.path.exists(input_dir[0]):
        raise ValueError(f"Could not find input dir for fold={fold} task={task} input_dir={input_dir}")
    ...
    with utils.Timer() as training:
        ...
        auto_sklearn.automl_ = auto_sklearn.build_automl(
            seed=seed,
            ensemble_size=auto_sklearn.ensemble_size,
            initial_configurations_via_metalearning=0,
            tmp_folder=auto_sklearn.tmp_folder,
            output_folder=auto_sklearn.output_folder,
        )
        ...
        for filename in files_to_copy:
            dst = filename.replace(input_dir, auto_sklearn.automl_._backend.temporary_directory + '/ ')
            os.makedirs(os.path.dirname(dst), exist_ok=True)
            if not os.path.exists(dst):
                copyfile(filename, dst)

        # Restore original run specific settings
        auto_sklearn.automl_._logger_port = logging.handlers.DEFAULT_TCP_LOGGING_PORT
        datamanager = auto_sklearn.automl_._backend.load_datamanager()
        auto_sklearn.automl_._task = datamanager.info['task']
        auto_sklearn.automl_._label_num = datamanager.info['label_num']
        dataset_name = datamanager.name
        y_train = auto_sklearn.automl_._backend.load_targets_ensemble()

        # Then finally fit the ensemble
        auto_sklearn.fit_ensemble(y_train, ensemble_size=50, dataset_name=dataset_name)

        # Cleanup this mess
        auto_sklearn.automl_._clean_logger()
        auto_sklearn.automl_._close_dask_client()
    ...
    overfit_frame = generate_overfit_artifacts(auto_sklearn, datamanager.data['X_train'], datamanager.data['Y_train'], X_test, Y_test)

...
def run(dataset, config):
    ...
    if 'fix_repetitions' in training_params:
        fix_repetitions = training_params.pop('fix_repetitions')
        if isinstance(fix_repetitions, str):
            fix_repetitions = True if 'true' in fix_repetitions else False
    ...
    auto_sklearn = estimator(time_left_for_this_task=config.max_runtime_seconds,
                             ...
                             resampling_strategy_arguments={'folds': k_folds,
                                                            ...
                                                            'repeats': repeats},
                             get_smac_object_callback=get_smac_object_callback(1 if fix_repetitions else repeats, max_stacking_level),
                             seed=config.seed,
                             **constr_extra_params,
                             **training_params)

...
class EnsembleTrajectorySimulator():
    ...
    def get_ensemble_performance(self, timestep):
        ...
        self.ensemble_selection.fit(
            np.array(self.ensemble_predictions[0: cutoff_ind]),
            self.labels, self.ensemble_identifiers[0:cutoff_ind])

        # get test performance
        if self.test_labels is not None:
            self.test_preds = self.ensemble_selection.predict(
                self.ensemble_predictions_test[0:cutoff_ind])
        ...
        if len(self.labels_ensemble_val) > 0 and len(self.ensemble_predictions_ensemble_val) > 0:
            ensemble_val_preds = self.ensemble_selection.predict(
                self.ensemble_predictions_ensemble_val[0:cutoff_ind])
...
def get_simulator_ensemble_config(metric):
...
def run(dataset, config):
    ...
    with utils.Timer() as training:
        ...
        simulator.save_trajectory(
            save_file=os.path.join(logdir, "ensemble_trajectory_test.json"), test=True)

    with utils.Timer() as predict:
        incumbent_score_val, incumbent_ind_val = simulator.get_incumbent_at_timestep(
            timestep=3600, use_val=True)

        predictions = np.array(simulator.test_preds)
        print(f"predictions are = {predictions}({np.shape(predictions)}) and GT={simulator.test_labels}({np.shape(simulator.test_labels)})   y_test={np.shape(y_test)}")

...
def fit_and_return_avg_predictions(
    args, X_train, y_train, X_test, y_test, model_name, repeat, splits,
):
    # Predicitions
    indices = [None] * splits * repeat
    oof_predictions = [None] * splits * repeat
    test_predictions = [None] * splits * repeat
    model = [None] * splits * repeat
    ...
    indices = np.concatenate([indices[i] for i in range(splits*repeat)
                              if indices[i] is not None])

    oof_predictions = np.concatenate([oof_predictions[i] for i in range(splits*repeat)
    ...
    # Convert to an average array
    test_predictions = [np.mean(test_predictions[splits*i:splits*(i+1)], axis=0) for i in range(0, repeat)]
...
def save_frame(args, repeated_frame, history_frame):
    ...
    path = os.path.join(os.getenv('HOME'), f"df_repeated_stacking_sklearn_feedback_frank_folds_{args.openml_id}_{args.seed}.csv")
    pd.DataFrame(repeated_frame).to_csv(path)
    print(f"{time.ctime()}: saved {path}")
    path = os.path.join(os.getenv('HOME'), f"df_repeated_stacking_sklearn_feedback_frank_folds_history_{args.openml_id}_{args.seed}.csv")
        ...
        for single_model in [False]:
            ...
            for level in range(0, 2):
                for model_name in list(HPO.keys()) + ['EnsembleSelection']:
                    ...
                    print(f"\nTry fitting {model_name} level={level} splits={splits} with single_model={single_model}")
                    try:

...
def fit_and_return_avg_predictions(
    ...
    for i, (train_index, val_index) in enumerate(rskf.split(X_train.copy(), y_train.copy())):
        ...
        val_predictions.append(balanced_accuracy_score(y_train[val_index], np.round(oof_predictions[i].argmax(1).reshape(-1, 1), decimals=0)))
    ...
    full_val_score = balanced_accuracy_score(y_train, np.round(np.mean(oof_predictions, axis=1), decimals=0))
    print(f"Produced {oof_predictions.shape} and {test_predictions.shape} with val score={full_val_score}")

    final_model = VotingClassifier(estimators=None, voting='soft', )
    final_model.estimators_ = model

    return np.round(oof_predictions.mean(axis=1), decimals=0).reshape(-1, 1), np.round(test_predictions.mean(axis=1), decimals=0).reshape(-1, 1), final_model, this_test_history, full_val_score, np.mean(val_predictions)



def save_frame(args, repeated_frame, history_frame):
    # Integrate that through progression
    path = os.path.join(os.getenv('HOME'), f"df_repeated_stacking_sklearn_avg_{args.openml_id}.csv")
    pd.DataFrame(repeated_frame).to_csv(path)
    print(f"{time.ctime()}: saved {path}")
    path = os.path.join(os.getenv('HOME'), f"df_repeated_stacking_sklearn_avg_history_{args.openml_id}.csv")
        ...
        for level in range(0, 6):
            for model_name in HPO.keys():
                ...
                try:
                    ...
                    oof_predictions_avg_repeat[model_name][level], test_predictions_avg_repeat[model_name][level], model, this_test_history, val_score, estimated_val_score = fit_and_return_avg_predictions(
                        args=args,
                    ...
                    df.append({
                        ...
                        'seed': args.seed,
                        'val_score': val_score,
                        'estimated_val_score': estimated_val_score,
                    })

...
def fit_and_return_ensemble_predictions(
    ...
    # Now fit a bunch of ensembles
    val_predictions = []
    test_predictions = []
    # The X_train = [[model1,repeat1, model1,repeat2]  [model2,repeat1, model2,repeat2, ...]]
    for repeat in range(len(X_train[0])):
        model = EnsembleSelection()
        model.fit(
            predictions=[pred[repeat] for pred in X_train],
            labels=y_train,
            identifiers=model_list,
        )
        val_predictions.append(model.predict([pred[repeat] for pred in X_train]))
        test_predictions.append(model.predict([pred[repeat] for pred in X_test]))

    val_score_repeatedTRUE = balanced_accuracy_score(y_train, np.mean(val_predictions, axis=0).argmax(1))
    test_score_repeatedTRUE = balanced_accuracy_score(y_test, np.mean(test_predictions, axis=0).argmax(1))
...
def save_frame(args, repeated_frame, history_frame):
    ...
    path = os.path.join(os.getenv('HOME'), f"df_repeated_stacking_sklearn_seed_ES_{args.openml_id}_{args.seed}.csv")
    pd.DataFrame(repeated_frame).to_csv(path)
    print(f"{time.ctime()}: saved {path}")
    path = os.path.join(os.getenv('HOME'), f"df_repeated_stacking_sklearn_seed_ES_history_{args.openml_id}_{args.seed}.csv")

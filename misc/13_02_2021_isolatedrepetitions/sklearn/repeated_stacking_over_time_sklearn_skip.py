...
def save_frame(args, repeated_frame, history_frame):
    ...
    path = os.path.join(os.getenv('HOME'), f"df_repeated_stacking_sklearn_skip_{args.openml_id}_{args.seed}.csv")
    pd.DataFrame(repeated_frame).to_csv(path)
    print(f"{time.ctime()}: saved {path}")
    path = os.path.join(os.getenv('HOME'), f"df_repeated_stacking_sklearn_skip_history_{args.openml_id}_{args.seed}.csv")
        ...
        for single_model in [True, False]:
            ...
            for level in range(0, 4):
                for model_name in HPO.keys():
                    ...
                    print(f"\nTry fitting {model_name} level={level} skip={skip} with single_model={single_model}")
                    try:
                        ...
                        if level > 0:
                            if not single_model:
                                if not skip:
                                    ...
                                    past_oof_prediction_aux = np.concatenate([
                                        oof_predictions_avg_repeat[model_name_aux][ilevel]
                                        for model_name_aux in oof_predictions_avg_repeat.keys() for ilevel in range(level)], axis=1)
                                    features_train = np.concatenate([
                                        features_train,  past_oof_prediction_aux], axis=1)
                                    past_test_prediction_aux = np.concatenate([
                                        test_predictions_avg_repeat[model_name_aux][ilevel]
                                        for model_name_aux in test_predictions_avg_repeat.keys() for ilevel in range(level)], axis=1)
                                    features_test = np.concatenate([
                                        features_test,  past_test_prediction_aux], axis=1)
                            else:
                                if not skip:
                                    ...
                                    past_oof_prediction_aux = np.concatenate([
                                        oof_predictions_avg_repeat[model_name][ilevel]
                                        for ilevel in range(level)], axis=1)
                                    features_train = np.concatenate([
                                        features_train,  past_oof_prediction_aux], axis=1)
                                    past_test_prediction_aux = np.concatenate([
                                        test_predictions_avg_repeat[model_name][ilevel]
                                        for ilevel in range(level)], axis=1)
                                    features_test = np.concatenate([
                                        features_test,  past_test_prediction_aux], axis=1)

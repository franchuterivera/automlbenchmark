...
def save_frame(args, repeated_frame, history_frame):
    ...
    path = os.path.join(os.getenv('HOME'), f"df_repeated_stacking_sklearn_rfonly_{args.openml_id}_{args.seed}.csv")
    pd.DataFrame(repeated_frame).to_csv(path)
    print(f"{time.ctime()}: saved {path}")
    path = os.path.join(os.getenv('HOME'), f"df_repeated_stacking_sklearn_rfonly_history_{args.openml_id}_{args.seed}.csv")
...
HPO = {
    ...
    'RandomForestClassifierAutoGluon': {
        'n_estimators': 300,
    },
...
model_func = {
    'RandomForestClassifierDefault': RandomForestClassifier,
    'RandomForestClassifierAutoSklearn': RandomForestClassifier,
    'RandomForestClassifierAutoGluon': RandomForestClassifier,
    ...
    'RandomForestClassifier4': RandomForestClassifier,
}

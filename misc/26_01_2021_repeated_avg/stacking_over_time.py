    ...
    try:
        ...
        for i, (train_index, val_index) in enumerate(rskf.split(X_train, y_train)):
            ...
            if i >= args.n_splits:
                ...
                start = ((i // args.n_repeats) - 1 ) * args.n_repeats
                print(f"range={range(start, start+args.n_repeats)}")
                past_off_prediction_idx = np.concatenate([oof_indices[i] for i in range(start, start+args.n_repeats)])
                past_off_prediction = np.concatenate([oof_predictions[i] for i in range(start, start+args.n_repeats)])
                ...
                past_test_prediction = np.mean([test_predictions[i] for i in range(start, start+args.n_repeats)], axis=0)
                print(f"features_test={features_test.shape} past_test_prediction={past_test_prediction.shape}")

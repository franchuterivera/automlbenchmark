...
for iterative in [True, False]:
    for framework in [
        ...
        for kaggle in [
            ...
            name = f"debug_run_{framework}_{iterative}_{kaggle}.sh"
            scripts.append(name)
            with open(name, 'w') as f:
                ...
                f.write(f"python run_kaggle.py  --iterative {iterative} -c 4 --runtime 3600 --half_for_test true -m 180G -t {kaggle} -f {framework}\n")
                f.write(f"cd $TMPDIR\n")
...
with open('launch.debug.csv', 'w') as launch:
    for item in scripts:
        launch.write(f"sbatch --bosch -p bosch_cpu-cascadelake -t 05:00:00 --mem 180G -c 4 --job-name {item.replace('.sh', '')} -o /home/riverav/AUTOML_BENCHMARK/automlbenchmark_fork/{item.replace('.sh', '')} /home/riverav/AUTOML_BENCHMARK/automlbenchmark_fork/misc/25_06_2021_run_kaggle/{item}\n")

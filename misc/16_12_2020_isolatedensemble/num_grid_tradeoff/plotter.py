...
def collapse(df: pd.DataFrame, by: typing.List[str] = ['tool', 'model', 'task', 'fold']) -> pd.DataFrame:
    """
    Collapses a dataframe that has multiple runs per seed
    """
    return df.groupby(by).mean().reset_index()
...
if __name__ == "__main__":
    ...
    # Collapse seed
    df = collapse(df, by=['tool', 'max_number_models', 'max_number_folds', 'max_num_repetitions', 'stacking_levels', 'cv_type', 'allowed_folds_to_use','task', 'model', 'fold'])
    # Collapse the fold
    df = collapse(df, by=['tool', 'max_number_models', 'max_number_folds', 'max_num_repetitions', 'stacking_levels', 'cv_type', 'allowed_folds_to_use','task', 'model'])
    df.to_csv('grid_tradeoff.csv')


    # Plotting time!
    for stacking in [1, 2]:
        for k in [3, 5, 10]:
            pvt = pd.pivot_table(df[(df['stacking_levels']==stacking) & (df['max_number_folds']==k)], values='test', index='max_number_models', columns='max_num_repetitions')
            ax=sns.heatmap(pvt, annot=True, fmt='.3g')
            ax.set_title(f"StackLevel={stacking} k-fold-cv k={k}")
            plt.show()
            plt.close()

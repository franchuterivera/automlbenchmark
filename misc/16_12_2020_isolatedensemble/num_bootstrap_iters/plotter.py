...
def parse_data(csv_location: str) -> pd.DataFrame:
    ...
    for data_file in glob.glob(os.path.join(csv_location, '*overfit.csv')):
        ...
        # SPlit tool into meaningful columns
        # autosklearnBBCScoreEnsemble_es50_B100_N1.0
        df[['framework_name', 'ensemble_size', 'BootstrapSize', 'BagSize']] = df['tool'].str.split('_', None, expand=True)
        data.append(df)
...
def collapse_fold(df: pd.DataFrame) -> pd.DataFrame:
    """
    Collapses a dataframe that has multiple runs per fold
    """
    # Collapse the seed
    return df.groupby(
        ['tool', 'model', 'task']
...
if __name__ == "__main__":
    ...
    for i, csv_location in enumerate(args.csv_location):
        ...
        df = collapse_seed(df)
        df = collapse_fold(df)
        print(f"raw_bootstrap.csv")
        df.to_csv('raw_bootstrap.csv')
